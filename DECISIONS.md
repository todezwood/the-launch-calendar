...
A third database, the **Agent Log**, gets one row per handled message, written after the reply so it adds no wait: kind (create / update / question / clarify / other), writes, held changes, duplicates refused, resends caught, error yes/no, seconds, and a 👍/👎 if someone reacts to the reply — every reply that saved something ends by asking for one, because nobody guesses a reaction is feedback. Its views are the **Hobbes performance** page. Again no message text and no exception text. It answers "is it fast, is it failing, do people agree with it". It cannot answer "was the update correct": that needs ground truth, which only the offline tests have (§5). A failure to log never fails a reply, and with no table configured the agent simply does not log.
